...
translation inspectable on fourteen dependently typed programming idioms.
\end{abstract}
...
\item We present fourteen showcased command batches (\S\ref{sec:overview},
...
translation. Each output comment below is transcribed
verbatim from the corresponding REPL golden file (\S\ref{sec:repl}). The
...
\texttt{cast<A => B>\^{}$\ell$(e)}, and the dynamic type as \texttt{Dyn}
(written $\dyn$ in the formal sections).
...
emits an extraction-failure blame (label~$2$), cast through the dynamic type~$\dyn$, and
applies it to the code. This is the translation's explicit fallback for ``a
value in type position,'' and our external-label theorem
...
\medskip
The lesson of these examples, and of the fourteen showcased batches in
...
trade-off. A blame label $p$ pairs an identifier with a
polarity. Identifier~$0$ is reserved for $\nu$-tampering, identifier~$1$ for
$\mathsf{is\_gnd}$-tampering, and identifier~$2$---written $\lint$
below---for extraction/coercion failure. Identifiers $\ge 3$ are external.
Keeping the three internal classes distinct preserves their diagnostic origin.
...
\end{theorem}
This is the invariant theorem formalized by the artifact. It is inspired by
the safety development in \emph{Blame for All}, but it is not presented as one
of that paper's numbered contextual corollaries.
...
binders, \texttt{cast<A => B>\^{}$\ell$(e)} for casts, \texttt{Dyn} for
$\dyn$). The paper's output comments are transcribed verbatim from the
checked-in golden files, which \texttt{dune test} replays. The main commands
are \texttt{Axiom}, \texttt{Definition}, \texttt{Check}, \texttt{Infer}, and
\texttt{Extract}, plus context management (\texttt{Delete}, \texttt{List});
\texttt{Check e.} without a type ascription behaves like \texttt{Infer}.
\texttt{Definition} is handled in the unverified shell by let-expansion:
later commands have their expressions wrapped in
$(\lambda x{:}T.\,\cdot)\;e$ for each definition they (transitively) use, so
the verified checker only ever sees the expanded term and definitional
transparency is ordinary $\beta$-conversion. Because the bare PTS has no
delta rule, a definition is \emph{opaque} at its use sites---which is exactly
why the example files axiomatize eliminators and define everything else
through them. \S\ref{sec:examples} lists complete input files.
...
Table~\ref{tab:examples} summarizes the fourteen showcase files shipped with
the development (three additional files test naming and error paths). Each is
a self-contained command batch processed by the REPL. Axioms introduce only
the inductive kit---type formers, constructors, and elimination/induction
principles---plus the value-as-type boundaries (\texttt{El}, \texttt{eval})
that would require large elimination, which the bare PTS lacks; every other
operation, from \texttt{vnth} through matrix multiplication and
\texttt{sprintf} to the proof steps of Newman's lemma, is an ordinary
\texttt{Definition} built from the eliminators.

\begin{table}
\small
\caption{The fourteen showcased examples. ``Erased'' names the dependency the extraction
removes; ``$\dyn$/blame'' marks examples that also exhibit the value-as-type
boundary.}
\label{tab:examples}
\Description{A table of fourteen example files and the dependent programming idiom each demonstrates.}
\begin{tabular}{@{}llc@{}}
\toprule
...
The complete Rocq development, the extracted REPL, and the fourteen example
