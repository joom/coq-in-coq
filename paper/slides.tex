...
\definecolor{noteNow}{HTML}{B45309}
\newcommand{\beathl}[1]{{\color{noteNow}\bfseries #1}}
...
\newcommand{\nb@item}[1]{\global\advance\nb@k by1
  \ifnum\nb@k=\nb@active\relax\beathl{#1}\else\beatdim{#1}\fi\par\smallskip}
\long\def\notescript#1{%
  \global\nb@total=0 \gdef\nb@list{}\gdef\nb@header{}%
  ...
  \begin{columns}[c,onlytextwidth]
    ...
    \begin{tikzpicture}[node distance=0.58cm]
      ...
      \node[rounded corners=3pt,inner sep=7pt,align=center,text width=3.15cm,
        ...
        below=0.5cm of boundary] (target) {%
      ...
      \draw[flow arrow,draw=tgtTeal] (boundary) -- (target);
    \end{tikzpicture}
  \end{columns}
  \note{%
    \textbf{[0:00 — 0:30]}\\
...
\begin{frame}{A format value can determine a function's type}
  ...
  \notescript{%
    \textbf{[0:30 — 1:30]}\\
    \beat{1}{Here is the only dependent-type idea you need for this talk. The
    format percent-d describes a function that consumes a natural number and
    returns a string. Percent-d percent-s describes a function with two
    arguments. The format is ordinary data, but it determines the function's
    type.}\\
    \lbeat{2}{That is what ``dependent'' means in this example: a value appears
    in, or computes, a type. The source type checker can therefore keep the
    format and every call site synchronized. You do not need to know the
    Calculus of Constructions to follow the rest; keep this one relationship in
    mind.}
  }
\end{frame}
...
\begin{frame}[fragile]{Rocq catches the drift}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{sourcecard}{\texttt{Fmt A} carries the promise}
      ...
      fint : Fmt A -> Fmt (Nat -> A)\\[0.55ex]
      fstr : Fmt A -> Fmt (Str -> A)
    \end{sourcecard}
  ...
  \notescript{%
    \textbf{[1:30 — 2:30]}\\
    \beat{1}{Here is the small amount of source notation behind it. A value of
    type \texttt{Fmt A} is a format denoting an \texttt{A}. Adding percent-d
    changes the index from \texttt{A} to \texttt{Nat to A}; percent-s does the
    same for strings. So \texttt{sprintf} can return exactly the function
    described by its format.}\\
...
\begin{frame}[fragile]{A plausible refactoring, a segfault}
  ...
  \notescript{%
    ...
    \lbeat{3}{This is not unique to printf. Seven of the eighteen dependent
...
\begin{frame}[fragile]{Evidence you can poke at}
  ...
  \notescript{%
    ...
    mathematical translation to OCaml, with a small REPL around them. The first
    command infers the source type of polymorphic identity; the second prints its
    target extraction.}\\
    \lbeat{2}{The repository includes eighteen golden example batches: vectors,
